				...
				for (int x = 0; x < width; x++)
				{
					...
					for (int l = 0; l <= SHOrder; l++)
					{
						for (int m = -l; m <= l; m++)
						{
							...
							SHCoeffiecents[sh::GetIndex(l, m)] += Le * sh * delta;
						}
					}
				}
			}
...
public:
	...
	virtual void preprocess(const Scene* scene) override
	{
		...
		for (int i = 0; i < mesh->getVertexCount(); i++)
		{
			...
			auto shFunc = [&](double phi, double theta) -> double
			{
				...
				const auto wi = Vector3f(d.x(), d.y(), d.z()); // 入射光
				double H = wi.dot(n);
				...
				else
				{
					...
					if (H > 0.0)
					{
						...
						// hit到其他地方说明光被挡住了，visibility = false
		...
		if (m_Type == Type::Interreflection)
		{
			...
			Eigen::Matrix3Xf indirectCoeffs;
			indirectCoeffs.resize(SHCoeffLength, mesh->getVertexCount());
			for (int i = 0; i < mesh->getVertexCount(); i++)
			{
				const auto& v = mesh->getVertexPositions().col(i);
				const auto& n = mesh->getVertexNormals().col(i).normalized();
				double* vertexCoeffs = indirectCoeffsSum(&m_TransportSHCoeffs, scene, v, n, 1);
	...
	double* indirectCoeffsSum(const MatrixXf* directCoeffs, const Scene* scene, const Point3f& v, const Normal3f& n, int depth = 0)
	{
		double res[SHCoeffLength] = { 0,0,0,0,0,0,0,0,0 };
		if (depth > m_Bounce)
			return res;

		// Monte Carlo: See sh::ProjectFunction
		...
		for (int t = 0; t < sample_side; t++)
		{
			for (int p = 0; p < sample_side; p++)
			{
				...
				if (H > 0.0)
				{
					Intersection intersection; // 交点
					Ray3f ray = Ray3f(v, wi); // 入射光
					bool hit = scene->rayIntersect(ray, intersection);
					if (hit)
					{
						const Mesh* mesh = intersection.mesh;
						Vector3f bary = intersection.bary; // 重心坐标
						Point3f triangleIndexs = intersection.tri_index;

						Point3f intersectPoint = intersection.p; //交点
						MatrixXf normals = mesh->getVertexNormals();
						Normal3f interNormal = Normal3f(
							normals.col(triangleIndexs.x()).normalized() * bary.x() +
							normals.col(triangleIndexs.y()).normalized() * bary.y() +
							normals.col(triangleIndexs.z()).normalized() * bary.z())
							.normalized();// 交点法线插值

						double* nextBounce = indirectCoeffsSum(directCoeffs, scene, intersectPoint, interNormal, depth + 1);
